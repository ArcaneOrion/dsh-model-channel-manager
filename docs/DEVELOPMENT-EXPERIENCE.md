...
> 本文沉淀 `@arcaneorion/dsh-model-channel-manager` 从 0 到 1 的完整开发经验与 DSH 模型渠道配置实践，包含大量从本会话真实踩坑中提炼的可复用结论。供后续迭代、新插件开发与日常模型配置参考。
...
### 3.6 提交前数据清洗（Data Sanitizer）

`settings.update` 提交 `llm-pi-ai` 前，必须清洗草稿：剔除空 id 的占位模型行、空字符串字段、非法 0 值超时等。否则底层 schemastery/zod 校验**整包拒绝**，表现为「保存无反应 / 保存失败」且无具体提示。
...
| 13 | 拖动供应商排序后刷新顺序还原 | `settings.update` 是 merge 语义：已存在键保序（对象 spread 旧键位置不动），字典键序无法靠 patch 改写 | 改用 `settings.mutate` path-ops：先 unset 全部键（顺带完成真删除），再按目标顺序 set 重插 |
...
DSH `llm-pi-ai` 官方三协议（`PROTOCOLS` 定义，缺一不可）：
...
切换协议的三个前提：上游必须真实暴露该格式端点；模型 ID 可能不同；baseURL 填根路径（协议层自动拼 `/v1/messages`）。

### 5.2 Provider 级字段

| 字段 | 用途 | 面板位置 |
| --- | --- | --- |
| `displayName` | 显示名（不影响 API） | 基础 |
| `baseURL` | 端点地址 | 基础 |
| `apiKeyEnv` | 凭据引用（环境变量名，不明文存 key） | 基础 |
| `models[]` | 模型列表 | 模型列表区 |
| `defaultContextWindow` / `defaultMaxTokens` / `defaultInput` | 未配模型时的兜底 | 高级选项 |
| `headers` | 自定义 HTTP 头（Key-Value） | 高级选项 |
...
- 优先上游确认「Prompt Cache 是否实现 + 是否回报 cached_tokens」—— 部分中转站不实现缓存，改配置也救不了。
